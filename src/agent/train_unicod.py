...
class TrainAgent:
    ...
    def __init__(self, cfg):
        ...
        if self.multi_gpu:
            ...
            for i in range(torch.cuda.device_count()):
                # log.info(f"Local rank: {local_rank}, GPU UUID: {torch.cuda.get_device_properties(i).uuid}")
                log.info(f"Local rank: {local_rank}, GPU UUID: {torch.cuda.get_device_properties(i).name}")
        ...
        if self.multi_gpu:
            log.info(f"Using {local_world_size} GPUs in each of the {cfg.n_nodes} nodes")
        ...
        self.grad_accumulation_steps = max(cfg.global_batch_size // cfg.per_device_batch_size // world_size, 1)
        actual_global_batch_size = (cfg.per_device_batch_size * self.grad_accumulation_steps * world_size)
        ...
        if self.run_eval:
            ...
            self.per_device_num_eval_batch = (cfg.eval_size // cfg.per_device_batch_size // world_size)
        ...
        if self.pred_dino:
            self.dino_type = cfg.dino_type
            self.trained_parameters += model.trainable_dino_expert_parameters
            self.dino_optimizer = bnb.optim.AdamW8bit(
                model.trainable_dino_expert_parameters,
                lr=cfg.dino_lr,
                weight_decay=cfg.dino_weight_decay,
            )
            self.dino_lr_scheduler = CosineAnnealingWarmupRestarts(
                self.dino_optimizer,
                first_cycle_steps=cfg.dino_lr_scheduler.first_cycle_steps,
                cycle_mult=1.0,
                max_lr=cfg.dino_lr,
                min_lr=cfg.dino_lr_scheduler.min_lr,
                warmup_steps=cfg.dino_lr_scheduler.warmup_steps,
                gamma=1.0,
            )
            log.info(f"Number of trained parameters (DINO): {get_num_params_in_billions(self.dino_optimizer):.3f}B")
        log.info(f"Number of trained parameters (Action): {get_num_params_in_billions(self.action_optimizer):.3f}B")
        if self.train_vlm:
            if cfg.lora:
            ...
            log.info(f"Number of trained parameters (VLM): {get_num_params_in_billions(self.vlm_optimizer):.3f}B")
    ...
    def run(self):
        ...
        cnt_update = (0 if not hasattr(self, "cnt_update") else self.cnt_update)  # resume training if loaded checkpoint
        loss_deque = deque(maxlen=self.grad_accumulation_steps)
        loss_action_deque = deque(maxlen=self.grad_accumulation_steps)
        if self.pred_dino:
            loss_dino_deque = deque(maxlen=self.grad_accumulation_steps)
        ...
        def preprocess_batch(batch, split_mask: bool, sample_fm_time: bool):
            ...
            images_chunk = batch["observation"]["image_primary"]
            # print("images_chunk", images_chunk)
            proprios_chunk = batch["observation"]["proprio"]
            actions_chunk = batch["action"].squeeze(1)  # remove the time dimension
            texts = [text.decode("utf-8") for text in batch["task"]["language_instruction"]]
            # print("images.shape", images.shape)  # 16 5 224 224 3
            # print("proprios.shape", proprios.shape)  # 16 5 7
            # print("actions.shape", actions.shape)  # 16 5 4 7
            # print("texts", texts)  # 16
            # print("actions", actions)
            # print("proprios", proprios)

            # use the first image and proprio as input, and predict the first action chunk and the last image (t=action_step+1), which should be the obs after executing the action chunk
            images = images_chunk[:, :1, :, :, :]
            proprios = proprios_chunk[:, :1, :]
            ...
            # print("image after processor", model_inputs["pixel_values"])
            ...
            if self.pred_dino:
                causal_mask, vlm_position_ids, dino_position_ids, proprio_position_ids, action_position_ids = (
                    model.build_causal_mask_and_position_ids(model_inputs["attention_mask"], self.dtype))
            else:
            ...
            if self.pred_dino:
                images_future = images_chunk[:, -1:, :, :, :]
                images_future = einops.rearrange(images_future,
                                                 "B T H W C -> B (T C) H W")  # remove cond_steps dimension
                if self.dino_type == "siglip" or self.dino_type == "siglip_w_encoder_decoder" or self.dino_type == "siglip_wo_enc" or self.dino_type == "siglip_imgloss":
                    model_inputs_future = self.processor(text=texts, images=images_future)
                    inputs["gt_dino_pixel_values"] = model_inputs_future["pixel_values"].to(self.dtype)
                    inputs["dino_pixel_values"] = model_inputs["pixel_values"].to(self.dtype)
                elif self.dino_type == "dino" or self.dino_type == "dino_future_distill":
                    from transformers import DINOv3ViTImageProcessorFast
                    dino_processor = DINOv3ViTImageProcessorFast.from_pretrained(
                        self.cfg.dino_model_path, local_files_only=True)
                    inputs["gt_dino_pixel_values"] = dino_processor(
                        images_future, return_tensors='pt')['pixel_values'].to(self.dtype)
                    inputs["dino_pixel_values"] = dino_processor(
                        images, return_tensors='pt')['pixel_values'].to(self.dtype)

                inputs["dino_position_ids"] = dino_position_ids
        ...
        while 1:
            for batch in self.train_dataloader:
                ...
                if (cnt_batch + 1) % self.grad_accumulation_steps != 0:
                    with model_meta.no_sync():
                        with torch.autocast(device_type="cuda", dtype=self.dtype, enabled=self.use_amp):
                            loss_dict = model_meta(**inputs)
                            loss_action = loss_dict["action_loss"]
                            if self.pred_dino:
                                loss_dino = loss_dict["dino_loss"]
                                loss = loss_action + loss_dino
                            else:
                                loss = loss_action
                        if self.debug:
                    ...
                    if self.debug:
                        log_allocated_gpu_memory(log, f"optimizer step batch {cnt_batch}")
                    ...
                    if (cnt_update % self.save_model_freq == 0 and
                            cnt_update > self.save_model_start) or cnt_update == self.n_updates:
                        self.save_training(cnt_update, cnt_batch, main_rank=self.main_rank)
                        dist.barrier()
                ...
                if self.multi_gpu:
                    ...
                    dist.all_reduce(loss_action, op=dist.ReduceOp.SUM)
                    loss_action_deque.append(loss_action.item() / dist.get_world_size())
                    if self.pred_dino:
                        dist.all_reduce(loss_dino, op=dist.ReduceOp.SUM)
                        loss_dino_deque.append(loss_dino.item() / dist.get_world_size())
                else:
                    loss_deque.append(loss.item())
                    loss_action_deque.append(loss_action.item())
                    if self.pred_dino:
                        loss_dino_deque.append(loss_dino.item())

                # validation with action accuracy
                if self.run_eval and (cnt_batch + 1) % self.eval_freq == 0:
                    log.info(f"Running evaluation for {self.per_device_num_eval_batch} batches...")
                ...
                if cnt_batch % self.log_freq == 0:
                    ...
                    loss_action_metric = np.mean(loss_action_deque)
                    if self.pred_dino:
                        loss_dino_metric = np.mean(loss_dino_deque)
                    peak_vram = torch.cuda.max_memory_reserved(self.gpu_id) / (1024**3)
                    log_msg = f"Batch {cnt_batch} Update {cnt_update}: t {timer():8.4f} | vram {peak_vram:6.3f} | loss {loss_metric:6.4f}"
                    if self.pred_dino:
                        log_msg += f" | action loss {loss_action_metric:6.4f} | dino loss {loss_dino_metric:6.4f}"
                        log_msg += f" | dino lr {self.dino_optimizer.param_groups[0]['lr']:10.8f}"
                    if self.train_vlm:
                    ...
                    if self.use_wandb:
                        ...
                        if self.pred_dino:
                            wandb_metrics["action loss"] = loss_action_metric
                            wandb_metrics["dino loss"] = loss_dino_metric
    ...
    def save_training(self, cnt_update: int, cnt_batch: int, main_rank: bool):
        ...
        data = {
            ...
            "vlm_optimizer": self.vlm_optimizer.state_dict() if self.train_vlm else None,
            "dino_optimizer": self.dino_optimizer.state_dict() if self.pred_dino else None,
            "action_lr_scheduler": self.action_lr_scheduler.state_dict(),
            "vlm_lr_scheduler": self.vlm_lr_scheduler.state_dict() if self.train_vlm else None,
            "dino_lr_scheduler": self.dino_lr_scheduler.state_dict() if self.pred_dino else None,
    ...
    def load_checkpoint(self, path: str):
        ...
        if self.pred_dino and (
            self.dino_type == "siglip_w_encoder_decoder"
            or self.dino_type == "siglip_wo_enc"
        ):
            self.model.load_state_dict(data["model"], strict=False)
            log.info(f"Loaded model from {path} and train from start")
        else:
            self.model.load_state_dict(data["model"], strict=True)
            self.cnt_update = data["cnt_update"]
            self.cnt_batch = data["cnt_batch"]
            self.wandb_id = data["wandb_id"]
            log.info(f"Loaded model from {path} at update {self.cnt_update} batch {self.cnt_batch}")
    ...
    def load_optimizer(self, path: str):
        ...
        if self.pred_dino:
            self.dino_optimizer.load_state_dict(data["dino_optimizer"])
            optimizer_to(self.dino_optimizer, self.device)
            self.dino_lr_scheduler.load_state_dict(data["dino_lr_scheduler"])
